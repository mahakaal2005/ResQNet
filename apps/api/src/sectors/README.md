...
`planZoneSplit()` is pure and writes nothing, so `MissionsService` validates a
zone *before* inserting the mission row. That is what makes an unusable zone a
clean 400 with no half-created mission to undo, and what keeps
`mission.created` ahead of `sector.assigned` in the audit trail.

Audited: `sector.assigned` once per zone split (one operator action, one row —
the labels are in `payload.sector_ids`), then `sector.created` or
`sector.updated` per `POST /sectors` depending on whether the label already
existed. Rejected writes record nothing. Vocabulary:
